...
import {
  brokerAddrFor,
  type AuthMode,
} from "../integration/fixture/fixture.js";
import {
...
const TRANSPORT = (process.env["CONFORMANCE_TRANSPORT"] ??
  "ws") as TransportType;
      ...
      async () => {
        ...
        if (TRANSPORT === "tcp") {
          ...
          const client = new Client({
            ...
            transport: "tcp",
            tokenProvider: () =>
              generateInvalidSignatureTestJwt(SECRET, AUDIENCE),
            timeout: 3000,
          });
          try {
            ...
            if (!connected) {
              evidence.push(
                "auth failure correctly set connected=false (TCP silent close)",
              );
              return { verdict: "pass", evidence };
            }
        ...
        try {
          ...
          if (caught) {
            evidence.push(
              `connect threw: ${(caught as Error).constructor.name}`,
            );
      ...
      async () => {
        ...
        await withClient({}, async (client) => {
          ...
          try {
            await client
              .rpc()
              .call(noWorkerRoute, b("ping"), { timeoutMs: 500 });
          } catch (err) {
            caught = err;
          }
          expect(caught).toBeTruthy();
          evidence.push(
            `rpc to unregistered route threw: ${(caught as Error).constructor.name}`,
          );
    ...
    const result = await runScenario(
      ...
      async () => {
        ...
        await withClient({}, async (client) => {
          ...
          expect(caught).toBeTruthy();
          evidence.push(
            `duplicate insert threw: ${(caught as Error).constructor.name}`,
          );
          ...
          evidence.push(
            "client remains usable after server-rejected operation",
          );
        });
    ...
    const result = await runScenario(
      ...
      async () => {
        ...
        try {
          ...
          expect(caught).toBeTruthy();
          evidence.push(
            `in-flight request threw: ${(caught as Error).constructor.name}`,
          );
          evidence.push(
            "in-flight request failed promptly on disconnect (correct)",
          );
        } finally {
          await workerClient.close().catch(() => undefined);
          await callerClient.close().catch(() => undefined);
      ...
      async () => {
        ...
        try {
          ...
          if (
            ...
            message.includes("Unknown config")
          ) {
        ...
        try {
          await withClient({}, async (client) => {
            ...
            if (records.length < 3) {
              verdict = "partial";
              evidence.push(
                `expected >=3 stream records, got ${records.length}`,
              );
            }
      ...
      async () => {
        ...
        await withClient({}, async (client) => {
          ...
          if (records.length < 2) {
            verdict = "partial";
            evidence.push(
              `expected >=2 records after commit, got ${records.length}`,
            );
          } else {
            evidence.push(
              `stream.read() completed cleanly with ${records.length} records`,
            );
          }
    ...
    const result = await runScenario(
      ...
      async () => {
        ...
        await withClient({}, async (client) => {
          ...
          expect(caught).toBeTruthy();
          evidence.push(
            `begin with wrong offset threw: ${(caught as Error).constructor.name}`,
          );
    ...
    const result = await runScenario(
      ...
      async () => {
        ...
        try {
          ...
          if (caught) {
            evidence.push(
              `in-flight begin threw: ${(caught as Error).constructor.name}`,
            );
